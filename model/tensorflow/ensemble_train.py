...
step_display = 50
step_save = 500
ensemble_path_save = './checkpoints/ensemble'
ensemble_start_from = ''
alexnet_start_from = './alexnet_bn'
resnet_start_from = './checkpoints/resnet-1000'
...
with tf.variable_scope("alexnet_model"):
    alexnet_logits = alexnet(x, keep_dropout, train_phase)

    # # Define loss and optimizer
    # alexnet_loss = tf.reduce_mean(tf.nn.sparse_softmax_cross_entropy_with_logits(labels=y, logits=alexnet_logits))
    # alexnet_train_optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(alexnet_loss)

    # # Evaluate model
    # alexnet_accuracy1 = tf.reduce_mean(tf.cast(tf.nn.in_top_k(alexnet_logits, y, 1), tf.float32))
    # alexnet_accuracy5 = tf.reduce_mean(tf.cast(tf.nn.in_top_k(alexnet_logits, y, 5), tf.float32))
num_classes = 100
with tf.variable_scope("resnet_model"):
    resnet_size = 18
    is_training = False
    resnet = resnet_model.imagenet_resnet_v2(resnet_size, num_classes)
    resnet_logits = resnet(x,is_training)
...
with tf.variable_scope("ensemble_model"):
    ...
    alexnet_train_optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss)
...
variables_to_restore = tf.get_collection(
            tf.slim.variables.VARIABLES_TO_RESTORE)
alexnet_varlist = {v.name[13:][:-2]: v 
                        for v in variables_to_restore if v.name[:12]=='alexnet_model'}
resnet_varlist = {v.name[12:][:-2]: v 
                        for v in variables_to_restore if v.name[:11]=='resnet_model'}

ensemble_varlist = {v.name[12:][:-2]: v 
                        for v in variables_to_restore if v.name[:11]=='ensemble_model'}

# define saver
alexnet_saver = tf.train.Saver(var_list=alexnet_varlist)
resnet_saver = tf.train.Saver(var_list=resnet_varlist)
ensemble_saver = tf.train.Saver(var_list=ensemble_varlist)
...
with tf.Session() as sess:
    ...
    while step < training_iters:
        ...
        if step % step_display == 0:
            ...
            print("-Iter " + str(step) + ", Training Loss= " + "{:.6f}".format(l) + ", Accuracy Top1 = " + 
                "{:.4f}".format(acc1) + ", Top5 = " + "{:.4f}".format(acc5))
            ...
            print("-Iter " + str(step) + ", Validation Loss= " + "{:.6f}".format(l) + ", Accuracy Top1 = " + 
                "{:.4f}".format(acc1) + ", Top5 = " + "{:.4f}".format(acc5))
