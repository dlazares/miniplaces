...
def capsnet(x,is_training,batch_size,output_size):
    ...
    with tf.variable_scope('PrimaryCaps_layer'):
        primaryCaps = CapsLayer(num_outputs=32, vec_len=8, with_routing=False, layer_type='CONV',batch_size=batch_size)
        caps1 = primaryCaps(conv1, kernel_size=9, stride=2)
        assert caps1.get_shape() == [batch_size, 1152, 8, 1]

    # DigitCaps layer, return [batch_size, 100, 16, 1]
    with tf.variable_scope('DigitCaps_layer'):
        digitCaps = CapsLayer(num_outputs=output_size, vec_len=16, with_routing=True, layer_type='FC',batch_size=batch_size)
